...
def main():
    ...
    # 初始化 YOLO 模型
    model_path = r"D:\mode\best_toy_20241109.pt"    # 請替換為你的模型路徑
    ...
    try:
                    ...
                    for mask in result.masks.data:
                        ...
                        if len(contours) > 0:
                            ...
                            if farthest_point1 is not None and farthest_point2 is not None:
                                ...
                                # 新增：標記最遠的兩個點並用紅線連接
                                cv2.circle(color_image, tuple(farthest_point1), 5, (0, 0, 255), -1)
                                cv2.circle(color_image, tuple(farthest_point2), 5, (0, 0, 255), -1)
                                cv2.line(color_image, tuple(farthest_point1), tuple(farthest_point2), (0, 0, 255), 2)
        ...
        if average_depth_object > 0 and object_center is not None:
            ...
            if screen_world_coords is not None:
                ...
                catch = relative_coords[2] / 2 
                if catch<0 :
                    catch= -catch
                print(f"夾取深度夾取深度({catch})")
