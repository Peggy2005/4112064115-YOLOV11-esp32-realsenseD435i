...
class ArmController:
    ...
    #adjusted_z = self.adjust_z_based_on_distance(object_coordinates[0], object_coordinates[1], object_coordinates[2])
      ...
      while object_coordinates[2] >= 30:
          ...
          print(1)
          timeout = time.time() + 5  # 設置5秒超時，避免無限迴圈
          print(2)
          while time.time() < timeout:
              current_position = self.bus_servo.get_position(1)
              print(current_position)
              if current_position >= 700:
                  print("Empty grab detected, lowering Z-axis and retrying...")
                  adjusted_z -= 10
                  if adjusted_z < 30:
                      print("Z-axis reached limit, stopping.")

                      return False

                  self.bus_servo.run(1, 700, 500)  # 再次嘗試夾爪閉合

                  time.sleep(0.5)

                  print(4)

              elif 590 <= current_position <= 610:

                  print("Successful grab detected.")
                  break
              time.sleep(0.1)
              print(5)
